...
function FormHdDatnDaihoc({
    dataSource,
    setDataSource
    // keKhaiTongHopId // Có thể không cần truyền trực tiếp nếu lưu ở form cha
}) {
    ...
    const handleEditItem = (record, index) => {
        ...
        form.setFieldsValue({
            ...
            tong_sv_quy_doi: record.tong_sv_quy_doi, // Nếu giảng viên tự nhập
    ...
    const calculateTongSvQuyDoi = (slCTTT, slDaiTra) => {
        // Theo file CSV, cột "Tổng số SV QĐ" có vẻ là tổng của CTTT và Đại trà
        // Nếu có quy tắc quy đổi khác, bạn cần cập nhật ở đây.
    ...
    const handleModalOk = async () => {
        try {
            ...
            const tong_sv_quy_doi_calculated = calculateTongSvQuyDoi(values.so_luong_sv_cttt, values.so_luong_sv_dai_tra);

            const newItem = {
                id_temp: editingItem?.record.id_temp || Date.now() + Math.random(),
                id_database: editingItem?.record.id_database || null,
                ...values,
                tong_sv_quy_doi: values.tong_sv_quy_doi !== undefined ? values.tong_sv_quy_doi : tong_sv_quy_doi_calculated, // Ưu tiên GV nhập, nếu không thì tính
            ...
            if (editingItem !== null && editingItem.index !== undefined) {
                ...
                message.success("Cập nhật thông tin hướng dẫn ĐATN/KLTN thành công.");
            } else {
                newData.push(newItem);
                message.success("Thêm mục hướng dẫn ĐATN/KLTN vào kê khai thành công.");
            }
    ...
    const columns = [
        ...
        },
        {
            ...
            width: 150,
            align: 'center',
            render: (text) => <Text strong style={{ color: '#52c41a' }}>{parseFloat(text || 0).toFixed(2)}</Text>
        },
    ...
    return (
        <div style={{ marginBottom: '20px', padding: '10px', border: '1px solid #f0f0f0', borderRadius: '8px' }}>
            ...
            <Button
                ...
                icon={<PlusOutlined />}
                style={{ marginBottom: 16, borderColor: '#52c41a', color: '#52c41a' }}
            >
            ...
            <Table
                ...
                bordered
                size="middle"
                locale={{ emptyText: 'Chưa có kê khai hướng dẫn ĐATN/KLTN Đại học.' }}
                summary={pageData => {
                        ...
                        dataSource && dataSource.length > 0 && (
                            <Table.Summary.Row style={{ background: '#fafafa' }}>
                                <Table.Summary.Cell index={0} colSpan={5} align="right">
                                    <Text strong>Tổng giờ Hướng dẫn ĐATN/KLTN:</Text>
                                </Table.Summary.Cell>
                                <Table.Summary.Cell index={1} align="center">
                                    <Text strong style={{ color: '#52c41a' }}>{totalGioQuyDoi.toFixed(2)}</Text>
                                </Table.Summary.Cell>
            ...
            <Modal
                title={editingItem ? `Chỉnh sửa Hướng dẫn ĐATN/KLTN` : `Thêm Hướng dẫn ĐATN/KLTN`}
                open={isModalVisible}
                onOk={handleModalOk}
                ...
                <Form form={form} layout="vertical">
                    ...
                    <Row gutter={16}>
                        <Col span={12}>
                            ...
                            >
                                <InputNumber min={0} style={{ width: '100%' }} placeholder="Nhập SL SV CTTT" />
                            </Form.Item>
                        </Col>
                        <Col span={12}>
                            <Form.Item
                                ...
                                rules={[{ type: 'number', min: 0, message: 'Số lượng không hợp lệ' }]}
                            >
                                <InputNumber min={0} style={{ width: '100%' }} placeholder="Nhập SL SV Đại trà" />
                            </Form.Item>
                        </Col>
                    </Row>
                    ...
                    <Form.Item
                        ...
                        rules={[{ type: 'number', min: 0, message: 'Tổng SV QĐ không hợp lệ' }]}
                    >
                        <Form.Item
                            noStyle
                            shouldUpdate={(prevValues, currentValues) =>
                                prevValues.so_luong_sv_cttt !== currentValues.so_luong_sv_cttt ||
                                prevValues.so_luong_sv_dai_tra !== currentValues.so_luong_sv_dai_tra
                            }
                        >
                            {({ getFieldValue, setFieldsValue }) => {
                                const slCTTT = getFieldValue('so_luong_sv_cttt') || 0;
                                const slDaiTra = getFieldValue('so_luong_sv_dai_tra') || 0;
                                const currentTongSVQD_Form = getFieldValue('tong_sv_quy_doi');
                                const calculatedTongSVQD = calculateTongSvQuyDoi(slCTTT,slDaiTra);

                                // Chỉ tự động cập nhật nếu người dùng chưa nhập hoặc giá trị tính toán khác
                                if (currentTongSVQD_Form === undefined || currentTongSVQD_Form === '' || parseFloat(currentTongSVQD_Form) !== calculatedTongSVQD) {
                                     // Để tránh vòng lặp vô hạn, chỉ set nếu giá trị khác
                                    if (form.getFieldValue('tong_sv_quy_doi') !== calculatedTongSVQD) {
                    ...
                    {/* Bỏ phần Upload minh chứng theo yêu cầu mới */}
                </Form>
            </Modal>
        </div>
    );
