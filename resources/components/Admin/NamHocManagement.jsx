...
function NamHocManagement() {
    ...
    const debouncedSearch = useCallback(
        debounce((searchValue) => {
            ...
            fetchNamHoc(1, pagination.per_page, searchValue);
        }, 500),
        [pagination.per_page]
    );
    ...
    useEffect(() => {
        if (searchTerm.trim() !== "") {
            ...
            fetchNamHoc(1, pagination.per_page, "");
        }
    ...
    const fetchNamHoc = async (page = pagination.current_page, perPage = pagination.per_page, search = searchTerm) => {
        setIsLoading(true);
        try {
            const params = {
                per_page: perPage,
    ...
    const handlePageChange = (page, pageSize) => {
        ...
        fetchNamHoc(page, pageSize, searchTerm);
    };
    ...
    const handleSubmit = async (values) => {
        ...
        try {
            if (editingId) {
                await axios.put(`/api/admin/nam-hoc/${editingId}`, values, {
                    headers: {
                        Authorization: `Bearer ${localStorage.getItem("token")}`,
                    },
                });
                ...
                await axios.post("/api/admin/nam-hoc", values, {
                    headers: {
                        Authorization: `Bearer ${localStorage.getItem("token")}`,
                    },
                });
    ...
    const handleImport = async () => {
        ...
        try {
            const response = await axios.post("/api/admin/nam-hoc/import", formData, {
                headers: {
                    Authorization: `Bearer ${localStorage.getItem("token")}`,
    ...
    const handleEdit = (namHoc) => {
        form.setFieldsValue({
            ten_nam_hoc: namHoc.ten_nam_hoc,
            la_nam_hien_hanh: namHoc.la_nam_hien_hanh.toString(),
        });
        setEditingId(namHoc.id);
    };
    ...
    const confirmDelete = async () => {
        ...
        try {
            console.log("Making DELETE request to:", `/api/admin/nam-hoc/${deletingId}`); // Debug log
            
            const response = await axios.delete(`/api/admin/nam-hoc/${deletingId}`, {
                headers: {
                    Authorization: `Bearer ${localStorage.getItem("token")}`,
                },
            });
            ...
            let errorMessage = "Có lỗi xảy ra khi xóa năm học";
    ...
    const downloadSampleFile = () => {
        ...
        link.setAttribute('download', 'mau_danh_sach_nam_hoc.csv');
    ...
    const columns = [
        ...
        {
            title: 'Trạng thái',
            dataIndex: 'la_nam_hien_hanh',
            key: 'la_nam_hien_hanh',
            width: 150,
            render: (la_nam_hien_hanh) => (
                la_nam_hien_hanh ? (
                ...
                <Card className="bg-white/95 backdrop-blur-lg border-gray-200/50 shadow-xl" style={{ borderRadius: '16px' }}>
                    <div className="flex items-center justify-between">
                        <div className="flex items-center space-x-6">
                            ...
                            <div className="space-y-2">
                                ...
                                <div className="flex items-center space-x-2">
                                    ...
                                    <Text type="secondary">Quản lý danh sách các năm học trong hệ thống</Text>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="flex items-center space-x-2 text-xs text-gray-400">
                            ...
                            <span className="text-purple-600 font-medium">Quản lý năm học</span>
                        </div>
                    </div>
                </Card>
                ...
                <Card className="bg-white/95 backdrop-blur-lg border-gray-200/50 shadow-xl" style={{ borderRadius: '16px' }}>
                    <div className="bg-gradient-to-r from-slate-50 via-blue-50/50 to-indigo-50/50 px-6 py-4 border-b border-gray-200/50 -mx-6 -mt-6 mb-6 rounded-t-2xl">
                        <div className="flex items-center">
                            ...
                            <div>
                                <Title level={4} style={{ margin: 0 }}>Tìm kiếm năm học</Title>
                                <Text type="secondary" className="text-sm">Tìm kiếm theo tên năm học</Text>
                            </div>
                        </div>
                    </div>
                        ...
                        <div className="flex items-center">
                            ...
                            <div>
                                ...
                                <Text type="secondary" className="text-sm">
                                    {editingId ? "Chỉnh sửa thông tin năm học hiện tại" : "Tạo năm học mới cho hệ thống"}
                                </Text>
                            </div>
                        </div>
                    </div>
                    ...
                    >
                        <Row gutter={[24, 24]}>
                            <Col xs={24} sm={12}>
                                <Form.Item
                                    name="ten_nam_hoc"
                                    label={<Text strong>Tên năm học</Text>}
                                    rules={[{ required: true, message: 'Vui lòng nhập tên năm học' }]}
                                >
                                    <Input
                                        size="large"
                                        placeholder="Nhập tên năm học (ví dụ: 2023-2024)"
                                        className="custom-input"
                                    />
                                </Form.Item>
                            </Col>
                            ...
                            <Col xs={24} sm={12}>
                                ...
                                >
                                    ...
                                    >
                                        <Option value="0">
                                            <div className="flex items-center">
                                                <Tag color="default" icon={<CloseCircleOutlined />} className="mr-2">
                                                    Không hiện hành
                                                </Tag>
                                            </div>
                                        </Option>
                    ...
                    <div className="bg-gradient-to-r from-slate-50 via-emerald-50/50 to-teal-50/50 px-6 py-4 border-b border-gray-200/50 -mx-6 -mt-6 mb-6 rounded-t-2xl">
                        <div className="flex items-center">
                            ...
                            <div>
                                ...
                                <Text type="secondary" className="text-sm">Tải lên file Excel hoặc CSV để thêm nhiều năm học</Text>
                            </div>
                        </div>
                    </div>
                            ...
                            <div className="mt-4 p-4 bg-blue-50 rounded-lg border border-blue-200">
                                <div className="flex items-start">
                                    ...
                                    <div className="text-xs text-blue-800 flex-1">
                                        ...
                                        <Text className="block mt-1">File phải có cấu trúc đúng với các cột: ten_nam_hoc, la_nam_hien_hanh</Text>
                                        <div className="mt-3 flex items-center justify-between">
                                            <Text className="text-blue-700">
                                                <strong>Ghi chú:</strong> la_nam_hien_hanh (0=Không hiện hành, 1=Năm hiện hành)
                                            </Text>
                                            <Button
                                                type="link"
                                                size="small"
                                ...
                                <div className="flex items-start">
                                    ...
                                    <div className="text-xs text-amber-800">
                                        ...
                                        <ul className="list-disc list-inside mt-1 space-y-1">
                                            ...
                                            <li>Tên năm học nên theo định dạng: YYYY-YYYY (VD: 2023-2024)</li>
                                            <li>Chỉ được có 1 năm học hiện hành (la_nam_hien_hanh = 1)</li>
                            ...
                            <div className="mt-3 p-3 bg-green-50 rounded-lg border border-green-200">
                                <div className="flex items-start">
                                    ...
                                    <div className="text-xs text-green-800">
                                        <Text strong className="block">Giá trị trạng thái (la_nam_hien_hanh):</Text>
                                        <div className="mt-2 grid grid-cols-1 gap-2">
                                            <div className="flex items-center space-x-2">
                                                <span className="font-mono text-green-700 bg-green-100 px-2 py-1 rounded">0</span>
                                                <span>Không hiện hành (năm học đã qua hoặc tương lai)</span>
                                            </div>
                                            <div className="flex items-center space-x-2">
                                                <span className="font-mono text-green-700 bg-green-100 px-2 py-1 rounded">1</span>
                                                <span>Năm hiện hành (năm học đang diễn ra)</span>
                                            </div>
                                        </div>
                                        <Text className="text-green-600 text-xs mt-2 block">
                                            <strong>Lưu ý:</strong> Hệ thống chỉ cho phép 1 năm học hiện hành tại một thời điểm
                                        </Text>
                                    </div>
                                </div>
                            </div>
                    ...
                    <div className="bg-gradient-to-r from-slate-50 via-purple-50/50 to-indigo-50/50 px-6 py-4 border-b border-gray-200/50 -mx-6 -mt-6 mb-6 rounded-t-2xl">
                        <div className="flex items-center justify-between">
                            <div className="flex items-center">
                                ...
                                <div>
                                    <Title level={4} style={{ margin: 0 }}>Danh sách năm học</Title>
                                    <Text type="secondary" className="text-sm">
                                        {searchTerm ? (
                                            `${pagination.total} kết quả tìm kiếm cho "${searchTerm}"`
                                        ) : (
                                            `${pagination.total} năm học trong hệ thống`
                                        )}
                                    </Text>
                                </div>
                            </div>
                            ...
                            emptyText: (
                                <div className="py-12 text-center">
                                    ...
                                    <Text className="text-gray-500 font-medium text-lg block">
                                        {searchTerm ? `Không tìm thấy năm học với từ khóa "${searchTerm}"` : "Chưa có năm học nào"}
                                    </Text>
                ...
                <Modal
                    title={
                        <div className="flex items-center">
                            ...
                            <span>Xác nhận xóa năm học</span>
                        </div>
                    }
...
export default NamHocManagement;
