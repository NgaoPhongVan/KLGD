...
function KhoaManagement() {
    ...
    const debouncedSearch = useCallback(
        debounce((searchValue) => {
            ...
            fetchKhoa(1, pagination.per_page, searchValue);
        }, 500),
        [pagination.per_page]
    );
    ...
    useEffect(() => {
        if (searchTerm.trim() !== "") {
            ...
            fetchKhoa(1, pagination.per_page, "");
        }
    ...
    const fetchKhoa = async (page = pagination.current_page, perPage = pagination.per_page, search = searchTerm) => {
        setIsLoading(true);
        try {
            const params = {
                per_page: perPage,
    ...
    const handlePageChange = (page, pageSize) => {
        ...
        fetchKhoa(page, pageSize, searchTerm);
    };
    ...
    const handleSubmit = async (values) => {
        ...
        try {
            if (editingId) {
                await axios.put(`/api/admin/khoa/${editingId}`, values, {
                    headers: {
                        Authorization: `Bearer ${localStorage.getItem("token")}`,
                    },
                });
                ...
                await axios.post("/api/admin/khoa", values, {
                    headers: {
                        Authorization: `Bearer ${localStorage.getItem("token")}`,
                    },
                });
    ...
    const handleImport = async () => {
        ...
        try {
            const response = await axios.post("/api/admin/khoa/import", formData, {
                headers: {
                    Authorization: `Bearer ${localStorage.getItem("token")}`,
    ...
    const handleEdit = (khoa) => {
        form.setFieldsValue({ ma_khoa: khoa.ma_khoa, ten_khoa: khoa.ten_khoa });
    ...
    const confirmDelete = async () => {
        ...
        try {
            ...
            let errorMessage = "Có lỗi xảy ra khi xóa khoa";
    ...
    const downloadSampleFile = () => {
        ...
        link.setAttribute('download', 'mau_danh_sach_khoa.csv');
                ...
                <Card className="bg-white/95 backdrop-blur-lg border-gray-200/50 shadow-xl" style={{ borderRadius: '16px' }}>
                    <div className="flex items-center justify-between">
                        <div className="flex items-center space-x-6">
                            ...
                            <div className="space-y-2">
                                ...
                                <div className="flex items-center space-x-2">
                                    ...
                                    <Text type="secondary">Quản lý danh sách các khoa trong trường</Text>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="flex items-center space-x-2 text-xs text-gray-400">
                            ...
                            <span className="text-purple-600 font-medium">Quản lý khoa</span>
                        </div>
                    </div>
                </Card>
                ...
                <Card className="bg-white/95 backdrop-blur-lg border-gray-200/50 shadow-xl" style={{ borderRadius: '16px' }}>
                    <div className="bg-gradient-to-r from-slate-50 via-blue-50/50 to-indigo-50/50 px-6 py-4 border-b border-gray-200/50 -mx-6 -mt-6 mb-6 rounded-t-2xl">
                        <div className="flex items-center">
                            ...
                            <div>
                                <Title level={4} style={{ margin: 0 }}>Tìm kiếm khoa</Title>
                                <Text type="secondary" className="text-sm">Tìm kiếm theo tên hoặc mã khoa</Text>
                            </div>
                        </div>
                    </div>
                        ...
                        <div className="flex items-center">
                            ...
                            <div>
                                <Title level={4} style={{ margin: 0 }}>
                                    {editingId ? "Cập nhật thông tin khoa" : "Thêm khoa mới"}
                                </Title>
                                <Text type="secondary" className="text-sm">
                                    {editingId ? "Chỉnh sửa thông tin khoa hiện tại" : "Tạo khoa mới cho hệ thống"}
                                </Text>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="bg-gradient-to-r from-slate-50 via-emerald-50/50 to-teal-50/50 px-6 py-4 border-b border-gray-200/50 -mx-6 -mt-6 mb-6 rounded-t-2xl">
                        <div className="flex items-center">
                            ...
                            <div>
                                ...
                                <Text type="secondary" className="text-sm">Tải lên file Excel hoặc CSV để thêm nhiều khoa</Text>
                            </div>
                        </div>
                    </div>
                            ...
                            <div className="mt-4 p-4 bg-blue-50 rounded-lg border border-blue-200">
                                <div className="flex items-start">
                                    ...
                                    <div className="text-xs text-blue-800 flex-1">
                                        ...
                                        <Text className="block mt-1">File phải có cấu trúc đúng với các cột: ma_khoa, ten_khoa</Text>
                                        <div className="mt-3 flex items-center justify-between">
                                            <Text className="text-blue-700">
                                                <strong>Lưu ý:</strong> Mã khoa phải duy nhất và không được trùng lặp
                                            </Text>
                                            <Button
                                                type="link"
                                                size="small"
                                ...
                                <div className="flex items-start">
                                    ...
                                    <div className="text-xs text-amber-800">
                                        ...
                                        <ul className="list-disc list-inside mt-1 space-y-1">
                                            ...
                                            <li>Đảm bảo mã khoa ngắn gọn và dễ nhớ</li>
                    ...
                    <div className="bg-gradient-to-r from-slate-50 via-purple-50/50 to-indigo-50/50 px-6 py-4 border-b border-gray-200/50 -mx-6 -mt-6 mb-6 rounded-t-2xl">
                        <div className="flex items-center justify-between">
                            <div className="flex items-center">
                                ...
                                <div>
                                    <Title level={4} style={{ margin: 0 }}>Danh sách khoa</Title>
                                    <Text type="secondary" className="text-sm">
                                        {searchTerm ? (
                                            `${pagination.total} kết quả tìm kiếm cho "${searchTerm}"`
                                        ) : (
                                            `${pagination.total} khoa trong hệ thống`
                                        )}
                                    </Text>
                                </div>
                            </div>
                            ...
                            emptyText: (
                                <div className="py-12 text-center">
                                    ...
                                    <Text className="text-gray-500 font-medium text-lg block">
                                        {searchTerm ? `Không tìm thấy khoa với từ khóa "${searchTerm}"` : "Chưa có khoa nào"}
                                    </Text>
